...
# =============================================================================
# =============================================================================
# Đọc các file RND và RND_ILP (phần này đã đúng)
# =============================================================================
...
# =============================================================================
# Sửa lại cách đọc cho các file FAIL, FAIL_ILP, RND_FAIL, RND_FAIL_ILP
# =============================================================================


# ---- Đọc nhiều file FAIL_ILP ----
all_files_fail_ilp = glob.glob(pathSimple + "Results_RND_FAIL_ILP_%s_*.csv" % time)
df3 = pd.concat([pd.read_csv(f) for f in all_files_fail_ilp], ignore_index=True)
...
# ---- Đọc nhiều file RND_FAIL ----
all_files_fail_r = glob.glob(pathSimple + "Results_RND_FAIL__%s_*.csv" % time)
df4 = pd.concat([pd.read_csv(f) for f in all_files_fail_r], ignore_index=True)
...
# ---- Đọc nhiều file RND_FAIL_ILP ----
all_files_ilp_fail_r = glob.glob(pathSimple + "Results_RND_FAIL_ILP_%s_*.csv" % time)
df5 = pd.concat([pd.read_csv(f) for f in all_files_ilp_fail_r], ignore_index=True)
...
ax.plot(ticks, QTYFails, '-',color='#a8b3a5')
ax.plot(ticks, QTYFailsILP, '-',color='#d8b365')
