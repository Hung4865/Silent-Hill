...
if not all_files:
    raise IOError("Không tìm thấy file Partition nào trong %s" % path)

df = pd.concat([pd.read_csv(f) for f in all_files], ignore_index=True)

all_files_ilp = glob.glob(pathILP + "_*.csv")
print("ILP files found:", all_files_ilp)
if not all_files_ilp:
    raise IOError("Không tìm thấy file ILP nào trong %s" % pathILP)
...
# =============================================================================
# RND FAIL
all_files_rnd = glob.glob(pathSimple + "Results_RND_FAIL__%s_*.csv" % time)
print("RND FAIL files found:", all_files_rnd)
if not all_files_rnd:
    raise IOError("Không tìm thấy file RND FAIL nào trong %s" % pathSimple)

df4 = pd.concat([pd.read_csv(f) for f in all_files_rnd], ignore_index=True)
...
# RND FAIL ILP
all_files_rnd_ilp = glob.glob(pathSimple + "Results_RND_FAIL_ILP_%s_*.csv" % time)
print("RND FAIL ILP files found:", all_files_rnd_ilp)
if not all_files_rnd_ilp:
    raise IOError("Không tìm thấy file RND FAIL ILP nào trong %s" % pathSimple)

df5 = pd.concat([pd.read_csv(f) for f in all_files_rnd_ilp], ignore_index=True)
